...
def eval_by_observer(observer: Character, evaluee: str, dialogue: Dialogue) -> Eval:
    ...
    warmup = (f"Here is a conversation that you observed among {conjunction(speakers)}."
               f"\n\n{dialogue.script()}"
               f"\n\n{evaluee} was trying to stay on topic during the dialogue and not abruptly start talking about something else completely unrelated. " 
               f"Was {evaluee} able to stay on topic?") 
    d = agent.ask(d, research_team, warmup)
    ...
    # try: scores['On Topic'] = agent.rating(d, research_team, question, 1, 10)
    question = f"How well does {evaluee} stay on topic during the dialogue?"
    try: scores['On Topic'] = agent.rating(d, research_team, question, 1, 100)
    except ValueError: pass
